...
阅读端不访问网络，资料库和图片均随 APK 打包（全量 `backend/data/ming_history.sqlite3`）。release 构建启用 R8 代码压缩与资源收缩；动态查找的图片资源由 `res/raw/keep.xml` 逐一保留。正式发布使用独立的 Android 签名密钥；密钥和密码不进入版本库，通过下列环境变量在本机或持续集成环境提供：
