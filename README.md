...
| `mingshi/` | 维基文库《明史》332 卷逐卷抓取（1739 年官修，公版，opencc 转简体） | 本纪/志/表/列传 | 10MB。**已知缺陷**：抓取只取 `<p>`，表部十三卷（世系表/年表）整张被丢，卷330–332 亦缺 |
| `mingshi_full/` | 全本《明史》txt（`ming_histroy.txt`，580 万字）经 `scripts/build_mingshi_full.py` 定形；表部与缺卷回维基文库按表格补入 | **332 卷全，343 万字**；本纪24／志75／表13／列传220 | 当前主源。定形做了四件事：切掉尾部 26,330 行重复块、剥 `</div>` 与分隔线、按句末标点接回被固定宽度劈开的句子、解双重编码的生僻字实体。仅卷39（历九）两源皆缺 |
